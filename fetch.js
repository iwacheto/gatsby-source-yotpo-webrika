...
exports.getShopifyProducts = exports.getQuestions = exports.getReviews = void 0;
...
const makeYotpoReviewRequest = async (yotpoAppKey, productId, yotpoPerPage, page) => {
  return await _axios.default.get(`https://api-cdn.yotpo.com/v1/widget/${yotpoAppKey}/products/${productId}/reviews.json`, {
    params: {
...
const getReviews = async ({
  ...
  try {
    reviews = await Promise.all(productIds.map(async productId => {
...
const makeYotpoQuestionRequest = async (yotpoAppKey, productId, yotpoPerPage) => {
  return await _axios.default.get(`https://api.yotpo.com/products/${yotpoAppKey}/${productId}/questions`, {
    params: {
      count: yotpoPerPage,
      page: 1
    }
...
const getQuestions = async ({
  ...
  const questions = await Promise.all(productIds.map(async productId => {
    const questionResponse = (await makeYotpoQuestionRequest(yotpoAppKey, productId, yotpoPerPage)).data.response;
...
const getShopifyProducts = async (shopName, token) => {
  ...
  if (response.products.pageInfo.hasNextPage) {
    ...
    response = await makeShopifyProductsRequest(shopName, token, lastProduct.cursor);
...
const makeShopifyProductsRequest = async (shopName, token, afterCursor) => {
  ...
  const results = await _axios.default.post(`https://${shopName}.myshopify.com/api/2019-07/graphql.json`, {
    query: `{
